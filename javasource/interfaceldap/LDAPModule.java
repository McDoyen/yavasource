...
import interfaceconfiguration.proxies.PersonImportScope;
...
public class LDAPModule {
	...
	protected LDAPObject convertFromDirectory(LDAPInterfaceConfiguration ldapInterfaceConfiguration, IContext context, TempLDAPObject tempLDAPObject, LdapTemplate ldapTemplate, String userNameField, ProcessingPart processingPart) throws MendixException 
	{
		...
		for (String property : tempLDAPObject.getPropertyNames()) {
			...
			if (property.equals(userNameField))
				attribute.setValue(tempLDAPObject.getTempLDAPAttribute(property).getValue());
				attribute.setMappable(tempLDAPObject.getTempLDAPAttribute(property).isMappable());
	...
	protected List<TempLDAPObject> getObjectChildren(String dn, String filter, LdapTemplate ldapTemplate, ProcessingPart processingPart, IContext context, LDAPInterfaceConfiguration ldapInterfaceConfiguration, LDAPPath ldapPath) {
		try{
			...
			int chunk = 1;
			do {
				List<TempLDAPObject> tempEntries = new LinkedList<TempLDAPObject>();
				if(pagedResultsDirContextProcessor.getResultSize() != 0){
					addConsoleLogging(Level.INFO, "Start retrieval of chunk " + chunk + ", page size is: " + pagedResultsDirContextProcessor.getPageSize() + ", query result size: " + pagedResultsDirContextProcessor.getResultSize());
				}
				else{
					addConsoleLogging(Level.INFO, "Start retrieval of chunk " + chunk + ", page size is: " + pagedResultsDirContextProcessor.getPageSize());
				}
				ldapTemplate.search(prepareDN(dn), filter, searchControls, new CustomNameClassPairCallbackHandler(tempEntries, context, processingPart, ldapInterfaceConfiguration, ldapPath), pagedResultsDirContextProcessor);
				if(pagedResultsDirContextProcessor.getResultSize() != 0){
					addConsoleLogging(Level.INFO, "Finished retrieval of chunk " + chunk + ", page size is: " + pagedResultsDirContextProcessor.getPageSize() + ", query result size: " + pagedResultsDirContextProcessor.getResultSize());
				}
				else{
					addConsoleLogging(Level.INFO, "Finished retrieval of chunk " + chunk + ", page size is: " + pagedResultsDirContextProcessor.getPageSize());
				}
				pagedResultsDirContextProcessor = new PagedResultsDirContextProcessor(pagedResultsDirContextProcessor.getPageSize(), pagedResultsDirContextProcessor.getCookie());
				chunk++;
	...
	public static void importLDAP(IContext context, ProcessingPart processingPart, LDAPInterfaceConfiguration ldapInterfaceConfiguration, List<LDAPImportType> ldapImportTypeList, LDAPImport ldapImport, boolean valueCheckChanged, String valueCheckCode, LDAPProcessingAttempt LDAPProcessingAttempt)  throws Exception 
	{
		for(LDAPImportType ldapImportType : ldapImportTypeList){
			...
			int total = 0;
			if (tempLDAPObjects != null) total = tempLDAPObjects.size();
			
			addConsoleLogging(Level.INFO, "Starting to process " + total + " records");
			int count = 0;
			
			for (TempLDAPObject tempLDAPObject : tempLDAPObjects) 
			{
				count++;
				if (count % 10 == 0) addConsoleLogging(Level.INFO, "Processed " + count + " of " + total + " records");
...
//				LdapModule.info("----------------------Started syncing user : " + dir.getProperty(userNameField));
				if (!tempLDAPObject.hasProperty(keyField)) {
					LDAPModule.addLogging(context, repeatingStructureProcessingPart, ldapInterfaceConfiguration, Level.WARNING, "Skipping (the ObjectGUID keyfield was not found in the imported object): " + tempLDAPObject.getModifiedDn());
					continue;
				}
				...
				String keyOld = "<error!>";
				try {
					key = tempLDAPObject.getTempLDAPAttribute(keyField).getValue();
					keyOld = tempLDAPObject.getTempLDAPAttribute(keyFieldOld).getValue();
				}
				...
				if (!context.isInTransaction()) context.startTransaction();
				
				boolean requiredFieldsNotMapped = false;
				...
				if(importEntity.equals(PersonLDAPInbox.entityName)){
					for(IMendixObject mapping: attrmappings){
						...
						if((ldapImportType.getPersonImportScope().equals(PersonImportScope.Import_Account)||ldapImportType.getPersonImportScope().equals(PersonImportScope.Import_Person_and_Account))){			
							if(userattr.equals(PersonAttribute.User_name.toString())){
								LDAPAttributeMapping userNameMapping = LDAPAttributeMapping.initialize(context, mapping);
								try {
									if(tempLDAPObject.getTempLDAPAttribute(userNameMapping.getLdapAttr()).getValue().equals("")||tempLDAPObject.getTempLDAPAttribute(userNameMapping.getLdapAttr()).getValue() == null){
										LDAPModule.addLogging(context, repeatingStructureProcessingPart, ldapInterfaceConfiguration, Level.WARNING, String.format("Skipping object %s, the username is required for accounts, it is mapped to LDAP attribute : %s, which is empty or not present in the object ", tempLDAPObject.getModifiedDn(), userNameMapping.getLdapAttr()));
										requiredFieldsNotMapped = true;
										continue;
									}
								}
								catch(Exception e) {
						...
						if((ldapImportType.getPersonImportScope().equals(PersonImportScope.Import_Person)||ldapImportType.getPersonImportScope().equals(PersonImportScope.Import_Person_and_Account))){
							if(userattr.equals(PersonAttribute.Last_name.toString())){
								LDAPAttributeMapping lastNameMapping = LDAPAttributeMapping.initialize(context, mapping);
								try{
									if(tempLDAPObject.getTempLDAPAttribute(lastNameMapping.getLdapAttr()).getValue().equals("")||tempLDAPObject.getTempLDAPAttribute(lastNameMapping.getLdapAttr()).getValue() == null){
										LDAPModule.addLogging(context, repeatingStructureProcessingPart, ldapInterfaceConfiguration, Level.WARNING, String.format("Skipping object %s, the lastname is required for persons, it is mapped to LDAP attribute : %s, which is empty or not present in the object ", tempLDAPObject.getModifiedDn(), lastNameMapping.getLdapAttr()));
										requiredFieldsNotMapped = true;
										continue;
									}
								}
					...
					for(IMendixObject mapping: attrmappings) 
					{
						...
						if (tempLDAPObject.hasProperty(ldapattr)){
							if(personAttributeString != null && !personAttributeString.equals("")){
								personLDAPInbox.getMendixObject().setValue(context, personAttributeString, tempLDAPObject.getTempLDAPAttribute(ldapattr).getValue());	
							}
							else{
					...
					for(IMendixObject mapping: attrmappings){
						String locationattr = mapping.getValue(context, LDAPAttributeMapping.MemberNames.ImportAttr.toString());
						if(locationattr.equals(LocationAttribute.Location_name.toString())){
							LDAPAttributeMapping locationNameMapping = LDAPAttributeMapping.initialize(context, mapping);
							if(tempLDAPObject.getTempLDAPAttribute(locationNameMapping.getLdapAttr()).getValue().equals("")||tempLDAPObject.getTempLDAPAttribute(locationNameMapping.getLdapAttr()).getValue() == null){
								LDAPModule.addLogging(context, repeatingStructureProcessingPart, ldapInterfaceConfiguration, Level.WARNING, String.format("Skipping object %s, the locationname is required for locations, it is mapped to LDAP attribute : %s, which is empty or not present in the object ", tempLDAPObject.getModifiedDn(), locationNameMapping.getLdapAttr()));
								requiredFieldsNotMapped = true;
								continue;
							}
						}
					...
					if(requiredFieldsNotMapped){
						repeatingStructureProcessingAttempt.setRepeatingStructureType(context, RepeatingStructuresType.LDAPLocationInbox);
						repeatingStructureProcessingAttempt.commit();							
						context.endTransaction();
						continue;
					}				
					...
					for(IMendixObject mapping: attrmappings) 
					{
						...
						if (tempLDAPObject.hasProperty(ldapattr)){
							if(locationAttributeString != null && !locationAttributeString.equals("")){
								locationLDAPInbox.getMendixObject().setValue(context, locationAttributeString, tempLDAPObject.getTempLDAPAttribute(ldapattr).getValue());
							}
							else{
								LDAPModule.addLogging(context, repeatingStructureProcessingPart, ldapInterfaceConfiguration, Level.WARNING, 
					...
					for(IMendixObject mapping: attrmappings){
						String organisationAttr = mapping.getValue(context, LDAPAttributeMapping.MemberNames.ImportAttr.toString());
						if(organisationAttr.equals(OrganisationAttribute.Organisation_name.toString())){
							LDAPAttributeMapping organisationNameMapping = LDAPAttributeMapping.initialize(context, mapping);
							if(tempLDAPObject.getTempLDAPAttribute(organisationNameMapping.getLdapAttr()).getValue().equals("")||tempLDAPObject.getTempLDAPAttribute(organisationNameMapping.getLdapAttr()).getValue() == null){
								LDAPModule.addLogging(context, repeatingStructureProcessingPart, ldapInterfaceConfiguration, Level.WARNING, String.format("Skipping object %s, the organisation name is required for organisations, it is mapped to LDAP attribute : %s, which is empty or not present in the object ", tempLDAPObject.getModifiedDn(), organisationNameMapping.getLdapAttr()));
								requiredFieldsNotMapped = true;
								continue;
							}
						}
					...
					if(requiredFieldsNotMapped){
						repeatingStructureProcessingAttempt.setRepeatingStructureType(context, RepeatingStructuresType.LDAPOrganisationInbox);
						repeatingStructureProcessingAttempt.commit();						
						context.endTransaction();
						continue;
					}
					...
					for(IMendixObject mapping: attrmappings) 
					{
						...
						if (tempLDAPObject.hasProperty(ldapattr)){
							if(organisationAttributeString != null && !organisationAttributeString.equals("")){
								organisationLDAPInbox.getMendixObject().setValue(context, organisationAttributeString, tempLDAPObject.getTempLDAPAttribute(ldapattr).getValue());
							}
							else{
								LDAPModule.addLogging(context, repeatingStructureProcessingPart, ldapInterfaceConfiguration, Level.WARNING, 
				...
				if (total == count) addConsoleLogging(Level.INFO, "Finished processing " + total + " records");
				context.endTransaction();
			}
		}
	}
	...
	private static Boolean getAccountActive(IContext context, ProcessingPart processingPart, LDAPInterfaceConfiguration ldapInterfaceConfiguration, TempLDAPObject tempLDAPObject){
		try {
			return new Boolean(tempLDAPObject.getTempLDAPAttribute("userAccountControl").getValue());
		}
		catch(Exception e) {
	...
	private static String getPersonAttribute(PersonAttribute personAttribute){
		switch(personAttribute){
			...
			case Time_Zone: return PersonLDAPInbox.MemberNames.TimeZone.toString();
			default: return null;
		}
	}
