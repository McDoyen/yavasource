	...
	private void handleSAMLResponse( SAMLRequestContext samlContext, MxSAMLResponse response ) throws SAMLException {
		...
		try {
			...
			if ( correspondingSAMLRequest == null && !ssoconfig.getAllowIdpInitiatedAuthentication() ) {
				...
				throw new SAMLFeedbackException(errorMessage).addFeedbackMessage("Unable to complete the request");
			}
			else {
				try {
					...
					if (ssoconfig.getIdentifyingAssertionType() == IdentifyingAssertionType.Use_Name_ID) {
						...
						try {
							...
							String errorMessage = "Unable to retrieve a principle key from SSO configuration, because of error: " + e.getMessage();
							SAMLUtil.createLogLine(errorMessage, SSOLogResult.Failed);
							throw new SAMLFeedbackException(errorMessage, e).addFeedbackMessage(Constants.ERROR_MESSAGE_NO_CONFIGURATION);
						}
						if ( principalKey != null ) {
							if ( assertionAttributes.containsKey(principalKey) ) {
								...
								String errorMessage = "The selected principal key " + principalKey + " in the configuration, isn't available in the response";
								SAMLUtil.createLogLine(errorMessage, SSOLogResult.Failed);
						...
						throw new SAMLFeedbackException("The response from the identity provider isn't valid.");
					}
					...
					try {
						...
						if ( samlSession != null ) {
							if ( samlSession.getIUser().isActive() ) {
								...
								if ( samlSession.getIUser().getUserRoleNames().size() == 0 ) {
									String errorMessage = "No userroles found for user '" + samlSession.getIUser().getName() + "'";
									SAMLUtil.createLogLine(errorMessage, SSOLogResult.Failed);
									throw new SAMLFeedbackException(errorMessage).addFeedbackMessage("Your account hasn't been configured to access this application.");
								}
								else {
								...
								throw new SAMLFeedbackException("Your account: " + samlSession.getIUser().getName() + " isn't active.");
							}
						}
						else {
							String errorMessage = "User lookup of '" + userPrincipal + "' failed, this userprincipal doesn't exist in the Mx database. ";
							SAMLUtil.createLogLine(errorMessage, SSOLogResult.Failed);
							throw new SAMLFeedbackException(errorMessage).addFeedbackMessage("The authentication was successful, but there is no account available in this application.");
						}
						...
						String errorMessage = "Couldn't create session for '" + userPrincipal + "': " + e.getMessage();
						SAMLUtil.createLogLine(errorMessage, SSOLogResult.Failed);
						throw new SAMLFeedbackException(errorMessage, e).addFeedbackMessage("An unexpected error occured while creating a session");
					}			
