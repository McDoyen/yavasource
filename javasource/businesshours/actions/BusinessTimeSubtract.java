...
public class BusinessTimeSubtract extends CustomJavaAction<java.util.Date>
{
	...
	public BusinessTimeSubtract(IContext context, java.util.Date Start, java.lang.Long Duration, IMendixObject BusinessTimesGroup, IMendixObject HolidaysGroup)
	{
		super(context);
		this.Start = Start;
	...
	try{
		...
		if (this.BusinessTimesGroup==null) { 
			Core.getLogger("BusinessHours").error("BusinessHoursCalculation: Subtract missing Business Times Group");
			return null;
		}
		if (this.Start==null) { 
			Core.getLogger("BusinessHours").error("BusinessHoursCalculation: Subtract missing Start");
			return null;
		}
		if (this.Duration==null) { 
			Core.getLogger("BusinessHours").error("BusinessHoursCalculation: Subtract missing Duration");
			return null;
		}
		Core.getLogger("BusinessHours").trace("BusinessHoursCalculation: Subtract " + this.Start + " - " + this.Duration +
		...
		Date result = calendarHours.subtractTime(start, this.Duration).getTime();
		Core.getLogger("BusinessHours").trace("BusinessHoursCalculation: Subtract result is " + result);
		return result;
		
	} catch (Exception e) {
		Core.getLogger("BusinessHours").error("Error calculating BusinessHours: "+e.getMessage());
		return null;
	}
		// END USER CODE
	}
