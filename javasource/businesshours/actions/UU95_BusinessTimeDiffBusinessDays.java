 ...
 * 
 */
public class UU95_BusinessTimeDiffBusinessDays extends CustomJavaAction<Long>
{
	...
	public UU95_BusinessTimeDiffBusinessDays(IContext context, java.util.Date Start, java.util.Date Stop, IMendixObject BusinessTimesGroup, IMendixObject HolidaysGroup)
	{
		super(context);
		this.Start = Start;
		this.Stop = Stop;
	...
	public Long executeAction() throws Exception
	{
		...
		if (this.BusinessTimesGroup==null) {
			Core.getLogger("BusinessHours").error("BusinessTimeDiffBusinessDays: Diff missing Business Times Group");
			return null;
		}
		if (this.Start==null) { 
			Core.getLogger("BusinessHours").error("BusinessTimeDiffBusinessDays: Diff missing Start");
			return null;
		}
		if (this.Stop==null) { 
			Core.getLogger("BusinessHours").error("BusinessTimeDiffBusinessDays: Diff missing Stop");
			return null;
		}
		Core.getLogger("BusinessHours").trace("BusinessTimeDiffBusinessDays: Diff " + this.Start + " - " + this.Stop +
		...
		for (Business_Time businessHoursSlot:BusinessTimesGroup.getBusiness_Time_Group_Business_Times(context)) {
			...
			Core.getLogger("BusinessHours").trace("BusinessTimeDiffBusinessDays: Slot from " + 
		...
		if (HolidaysGroup!=null) {
			List<Holiday> listHolidays = this.HolidaysGroup.getHoliday_Group_Holidays(context);
			if (listHolidays!=null && listHolidays.size()>0) {
				for (Holiday holiday:listHolidays) {
					...
					Core.getLogger("BusinessHours").trace("BusinessTimeDiffBusinessDays: Adding holiday " + holiday.getHolidayDate());
				}
			}
		}
		...
		stop.setTime(this.Stop);
		 
		long result = calendarHours.diffTimeBusinessDays(start, stop);
		Core.getLogger("BusinessHours").trace("BusinessTimeDiffBusinessDays: Diff result is " + result);
		return result;
	
	} catch (Exception e) {
		Core.getLogger("BusinessHours").error("BusinessTimeDiffBusinessDays: Error calculating BusinessHours: "+e.getMessage());
		return null;
	}
	...
	{
		return "BusinessTimeDiffBusinessDays";
	}
