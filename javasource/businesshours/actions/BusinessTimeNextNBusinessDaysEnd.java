...
public class BusinessTimeNextNBusinessDaysEnd extends CustomJavaAction<java.util.Date>
{
	...
	public BusinessTimeNextNBusinessDaysEnd(IContext context, java.util.Date Start, java.lang.Long Days, IMendixObject BusinessTimesGroup, IMendixObject HolidaysGroup)
	{
		super(context);
		this.Start = Start;
		this.Days = Days;
	...
	public java.util.Date executeAction() throws Exception
	{
		...
		if (this.BusinessTimesGroup==null) {
			Core.getLogger("BusinessHours").error("BusinessHoursCalculation: Next N days end missing Business Times Group");
			return null;
		}
		if (this.Start==null) {
			Core.getLogger("BusinessHours").error("BusinessHoursCalculation: Next N days end missing Start");
			return null;
		}
		if (this.Days==null) { 
			Core.getLogger("BusinessHours").error("BusinessHoursCalculation: Next N days end missing Days");
			return null;
		}
		if (this.Days<0) { 
			Core.getLogger("BusinessHours").error("BusinessHoursCalculation: Next N days end Days should be positive");
			return null;
		}
		Core.getLogger("BusinessHours").trace("BusinessHoursCalculation: End of day after " + this.Days + " days since " + this.Start + 
		...
		Date result = calendarHours.nextNBusinessDaysEnd(start, this.Days.intValue()).getTime();
		Core.getLogger("BusinessHours").trace("BusinessHoursCalculation: Next N days end result is " + result);
		return result;
	...
	{
		return "BusinessTimeNextNBusinessDaysEnd";
	}
