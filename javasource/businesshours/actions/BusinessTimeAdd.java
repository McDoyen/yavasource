...
public class BusinessTimeAdd extends CustomJavaAction<java.util.Date>
{
	...
	public BusinessTimeAdd(IContext context, java.util.Date Start, java.lang.Long Duration, IMendixObject BusinessTimesGroup, IMendixObject HolidaysGroup)
	{
		super(context);
		this.Start = Start;
	...
	try{
		...
		if (this.BusinessTimesGroup==null) {
			Core.getLogger("BusinessHours").error("BusinessHoursCalculation: Add missing Business Times Group");
			return null;
		}
		if (this.Start==null) {
			Core.getLogger("BusinessHours").error("BusinessHoursCalculation: Add missing Start");
			return null;
		}
		if (this.Duration==null) { 
			Core.getLogger("BusinessHours").error("BusinessHoursCalculation: Add missing Duration");
			return null;
		}
		Core.getLogger("BusinessHours").trace("BusinessHoursCalculation: Add " + this.Start + " + " + this.Duration +
		...
		Date result = calendarHours.addTime(start, this.Duration).getTime();
		Core.getLogger("BusinessHours").trace("BusinessHoursCalculation: Add result is " + result);
		return result;
		
	} catch (Exception e) {
		Core.getLogger("BusinessHours").error("Error adding BusinessHours: "+e.getMessage());
		return null;
	}
		// END USER CODE
	}
